...
{{--    </div>--}}
    ...
    <div class="col-sm-2 mb-3">
        ...
        <div class="dropdown-menu" role="menu">
            <a class="dropdown-item @if(!isset($_GET['time']) or $_GET['time'] == 1) active @endif" href="{{ route('creator.films.index',['genre'=>$get_genre,'time'=>1]) }}">1 месяц</a>
            <a class="dropdown-item @if(isset($_GET['time']) and $_GET['time'] == 3) active @endif" href="{{ route('creator.films.index',['genre'=>$get_genre,'time'=>3]) }}">3 месяца</a>
            <a class="dropdown-item @if(isset($_GET['time']) and $_GET['time'] == 6) active @endif" href="{{ route('creator.films.index',['genre'=>$get_genre,'time'=>6]) }}">6 месяцeв</a>
            <a class="dropdown-item @if(isset($_GET['time']) and $_GET['time'] == 12) active @endif" href="{{ route('creator.films.index',['genre'=>$get_genre,'time'=>12]) }}">1 год</a>
            <a class="dropdown-item @if(isset($_GET['time']) and $_GET['time'] == 'all') active @endif" href="{{ route('creator.films.index',['genre'=>$get_genre,'time'=>'all']) }}">Все</a>
        </div>
    </div>
</div>
