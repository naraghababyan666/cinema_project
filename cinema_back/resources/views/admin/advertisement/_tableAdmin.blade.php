...
{{--            {{ $item->user->email ?? "--"}}--}}
    ...
    <td>
        <a href="{{ 'http://'.$_SERVER['HTTP_HOST']."/storage/videos/".$item->file }}">Видео ролик</a>
{{--        <p>{{ 'http://'.$_SERVER['HTTP_HOST']."/storage/".$item->file }}</p>--}}
    </td>
    <td class="text-center">
        {{ $item->created_at ?? '--' }}
    </td>
    <td class="text-center">
        ...
        @else
            <div style="display: flex; justify-content: center;">
                <form action="{{route('admin.advertisement.change', ['id' => $item->id, 'value' => 1])}}" method="POST">
                    @csrf
                ...
                <form action="{{route('admin.advertisement.change', ['id' => $item->id, 'value' => 2])}}" method="POST">
                    @csrf
                    <button type='submit' class="text-primary mx-2" style="border: none; background: transparent">
