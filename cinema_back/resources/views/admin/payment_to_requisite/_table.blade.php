    ...
    <td class="text-center">
        ...
        @else
            <div style="display: flex; justify-content: center;">
                <form action="{{route('admin.payment_to_requisite.change', ['id' => $item->id, 'value' => 1, 'user_id' => $item->user->id, 'wanted_total' => $item->wanted_total])}}" method="POST">
                    @csrf
                    @method('POST')
                ...
                <form action="{{route('admin.payment_to_requisite.change', ['id' => $item->id, 'value' => 2])}}" method="POST">
                    @csrf
                    @method('POST')
                    <button type='submit' class="text-primary mx-2" style="border: none; background: transparent">
