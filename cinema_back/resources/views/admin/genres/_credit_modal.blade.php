...
<div class="modal fade" id="modal{{ $id }}show" tabindex="-1" role="dialog" aria-labelledby="modal{{ $id }}showLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form action="@isset($item){{ route('admin.genres.update',['id'=>$item->id]) }}@else{{ route('admin.genres.store') }}@endisset" class="was-validated" method="POST" enctype="multipart/form-data">
                @isset($item) @method('PATCH') @endisset
                @csrf
                <div class="modal-header">
                    <h5 class="modal-title" id="modal{{ $id }}showLabel">@isset($data)Редактирование @else Создание @endisset жанра</h5>
